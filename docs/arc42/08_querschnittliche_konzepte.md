...
Im aktuellen Stand wird pro HTTP-Request genau eine `correlation_id` erzeugt. Dieselbe ID erscheint bei Fehlerantworten, wird bei neu erzeugten Runs mitpersistiert und über die Run-API wieder sichtbar gemacht. Zusätzlich setzt die API sie als `X-Correlation-ID` Response-Header und schreibt beim Abschluss des Requests einen strukturierten Logeintrag mit Methode, Pfad, Statuscode und `correlation_id`. Dadurch ist ein kleiner, aber klarer Request-to-Run-Nachvollziehbarkeitspfad vorhanden.

Diese Felder und Logkontexte reichen für die Nachvollziehbarkeit einzelner Runs im MVP bereits weit, bilden aber noch keinen durchgängigen Audit-Kontext über alle Schichten. Ein Run ist damit gut erklärbar; ein vollständiger Audit-Trail über Browser, API, Datenbank, Logs und externen Provider hinweg ist jedoch noch nicht vorhanden. Die Laufnachvollziehbarkeit ist dadurch deutlich verbessert, ersetzt aber keine produktionsreife Observability.
